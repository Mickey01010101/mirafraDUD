...
*/ 	copy_to_user(buff,bu,sizeof(bu));
	printk("data is :%s\n",bu);
return 0;
}
...
{	
	...
	//bu=(char *)buff;
	copy_from_user(bu+*off,buff,len);
	*off=*off+len;
	return 0;
}
...
static int __init chr_driver_start(void)
{
	...
	if(alloc_chrdev_region(&cdev,0,1,"drv")<0)
	{
...
/* this is optional becoz u dint not enter every time insomd simply it can automatically 
        dev_class=class_create(THIS_MODULE,"my_drv");
        device_create(dev_class,NULL,cdev,NULL,"drv") */
	return 0;
}

static void __exit chr_driver_exit(void)
{
/* this is optional becoz u dint use rmnod in every time driver removing 
